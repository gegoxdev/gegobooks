...
import { UserPlus, Trash2, ChevronDown, Shield, ShieldCheck, ShieldAlert, Eye } from 'lucide-react';
...
const AdminManagement = ({ currentRole }: { currentRole: string }) => {
  ...
  const [addRole, setAddRole] = useState('readonly');
  ...
  const handleAddAdmin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      toast.success(`Added ${addEmail.trim()} as ${roleConfig[addRole].label}`);
      setAddEmail('');
      setAddRole('readonly');
      fetchAdmins();
    }
  ...
  const handleRemove = async (admin: AdminUser) => {
    ...
    setRemovingId(admin.admin_id);

    if (admin.user_id) {
      const { error } = await supabase.rpc('admin_remove_admin' as any, { target_user_id: admin.user_id });
      if (error) toast.error(error.message);
      else { toast.success(`Removed ${admin.email}`); fetchAdmins(); }
    } else {
      const { error } = await supabase.rpc('admin_remove_admin_by_email' as any, { target_email: admin.email });
      if (error) toast.error(error.message);
      else { toast.success(`Removed ${admin.email}`); fetchAdmins(); }
    }
    setRemovingId(null);
  ...
  return (
    <div className="bg-surface rounded-xl border border-border p-6">
      ...
      {/* Add admin form - master only */}
      {isMaster && (
        <form onSubmit={handleAddAdmin} className="flex flex-col sm:flex-row gap-3 mb-4">
          <input
            type="email"
                  ...
                  <tr key={a.admin_id} className="border-b border-border/50 hover:bg-soft-white transition-colors">
                    ...
                    <td className="py-3 px-2 relative">
                      {isMaster ? (
                        <>
                          <button
                            onClick={() => setEditingId(editingId === a.admin_id ? null : a.admin_id)}
                    ...
                    <td className="py-3 px-2">
                      <span className={`font-body text-xs px-2 py-0.5 rounded-full font-medium ${
                        hasSignedIn ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                      }`}>
                    ...
                    {isMaster && (
                      <td className="py-3 px-2">
                        <button
                          ...
                          disabled={removingId === a.admin_id}
      ...
      <div className="mt-4 pt-4 border-t border-border">
        ...
        <p className="font-body text-xs text-muted mt-2">
          Master = full control • Admin = manage data • Approver = view • Read Only = view only
        </p>
      </div>
    </div>
  );
